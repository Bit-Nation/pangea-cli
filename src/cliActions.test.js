...
const testKeyPair = {
    publicKey: new Uint8Array('40c40e5d1d86be8ee8beb08ad1346666e7504315d335b051610067e7a9a61cc4'.match(/.{1,2}/g).map(byte => parseInt(byte, 16))),
...
const testSigningKey = {
    ...
    "private_key_cipher_text": {
        ...
        "p": 1,
        "dk_len": 32,
        "salt": "",
        "cipher_text": "",
...
describe('cliActions', () => {
    ...
    describe('newSigningKey', () => {
        ...
        test('password miss match', (done) => {
            ...
            newSigningKey({pw: "my password", pwConfirmation: "wrong password confirmation"})
                .then(() => {
                    done.fail('expected tests to fail since we passed in unequal passwords')
                })
                .catch((err) => {
                    expect(err.message).toBe('password and password confirmation have to match');
                    done();
                })
        ...
        test('generate successfully', (done) => {
            ...
            fs.writeFile.mockImplementation((fileName, signingKeyStr, cb) => {
                ...
                // generated key pair
                expect(singingKey.public_key).toBe(Buffer.from(testKeyPair.publicKey).toString('hex'));

                decryptValue(singingKey.private_key_cipher_text, 'my password')
                    .then((decryptedValue) => {
                        // make sure decrypted secret key is really the secret key
                        expect(decryptedValue.toString()).toBe(Buffer.from(testKeyPair.secretKey).toString('hex'));
                        cb()
                    })
                    .catch(cb)
    ...
    describe('signingKeyChangePW', () => {
        ...
        test('new password miss match', (done) => {
            ...
            signingKeyChangePW({newPw: "my_new_password", newPwConfirmation: "wrong confirmation"})
                .then(() => {
                    done.fail("exepcted test to fail since the file doesn't exist")
                })
                .catch((err) => {
