const {encryptValue, decryptValue, isInvalidValidPassword} = require('./utils');
    ...
    test('encrypt', (done) => {
        ...
        encryptValue('value to encrypt', 'password')
            .then((encryptedValue) => {
                ...
                scrypt('password', Buffer.from(salt, 'hex'), {
                    N: n,
                    r: r,
                    p: p,
                    dkLen: dk_len,
                    ...
                    const plainValue = aes.utils.utf8.fromBytes(ctr.decrypt(Buffer.from(cipher_text, 'hex')));
    ...
    test('decrypt successfully', (done) => {
        ...
        crypto.randomBytes(20, (err, salt) => {
            ...
            scrypt('password', Buffer.from(salt, 'hex'), {
                ...
                const ctr = new aes.ModeOfOperation.ctr(derivedSecret);
                const hexCipherText = aes.utils.hex.fromBytes(ctr.encrypt(aes.utils.utf8.toBytes(plainValue)));
            ...
            scrypt('password', Buffer.from(salt, 'hex'), {
                ...
                decryptValue(encryptedValue, 'wrong password')
                    .then(() => {
                        ...
                        done.fail('expected the test to fail since we are using an invalid password');
        ...
        const expectations = [
            {
                ...
                expectation: (validationResult) => {
                    expect(validationResult.message).toBe('password must have at least 8 characters')
                },
            },
            {
